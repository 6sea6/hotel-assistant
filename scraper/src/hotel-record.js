...
function buildNotes(template, scraped, transit, roomOverride, roomDetails = extractRoomOutputDetails(roomOverride || (scraped && scraped.room) || {})) {
  const parts = [];
  ...
  const spiderErrorCodes = Array.from(new Set(
    (((scraped && scraped.page_snapshot && scraped.page_snapshot.sources) || []))
      .flatMap((source) => Array.isArray(source && source.spider_error_codes) ? source.spider_error_codes : [])
  ...
  if (selectedRoomLocked) {
    appendUniqueNote(parts, '警告: 当前匹配房型被携程标记为“登录看低价/隐藏价格”，该目标房型在当前抓取会话下价格仍不可见。');
  }

  if (!room || room.price === null || room.price === undefined) {
    appendUniqueNote(parts, selectedRoomLocked
  ...
  if (spiderErrorCodes.length > 0 && !selectedRoomHasPrice) {
    appendUniqueNote(parts, `警告: 携程房型接口触发反爬限制，错误码 ${spiderErrorCodes.join(', ')}，本次未能获取可用房价。`);
  }
...
function createRecordContext(template, transit, matchedTemplate, now = new Date().toISOString()) {
  return {
    ...
    days: differenceInDays(template && template.check_in_date, template && template.check_out_date) || (template && template.days),
...
function buildRecordFromRoom(template, scraped, matchedTemplate, room, context, options = {}) {
  ...
  const templateRoomCount = toNumber((matchedTemplate && matchedTemplate.room_count) || (template && template.room_count)) || 1;
  ...
  return {
    ...
    destination: normalizePlaceName((matchedTemplate && matchedTemplate.destination) || (template && template.destination)),
    distance: context.route ? toStringNumber(context.route.distanceKm, 1) : '',
    subway_station: (context.nearestSubway && context.nearestSubway.distanceKm <= 1.5) ? context.nearestSubway.name : '无',
    subway_distance: (context.nearestSubway && context.nearestSubway.distanceKm <= 1.5) ? String(context.nearestSubway.distanceKm) : '0',
    ...
    room_type: normalizeText((room && room.standard_title) || (room && room.title) || (template && template.room_type)),
    original_room_type: normalizeText((room && room.original_title) || (room && room.title)) || '',
    room_count: roomOccupancy || templateRoomCount,
    room_area: toStringNumber(roomDetails.areaValue !== null ? roomDetails.areaValue : (room && room.area), 0),
...
function buildEligibleRoomRecords(template, scraped, transit, matchedTemplate) {
  const eligibleRooms = Array.isArray(scraped && scraped.eligible_rooms) ? scraped.eligible_rooms : [];
  if (eligibleRooms.length === 0) {
    return [];
  }

  const context = createRecordContext(template, transit, matchedTemplate);
  return eligibleRooms.map((room) => buildRecordFromRoom(
    template,
    scraped,
    matchedTemplate,
    room,
