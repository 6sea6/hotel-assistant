function createSessionSettleBrowserHelpers() {
  ...
  const getSelectorScanState = () => {
    if (!window.__ctripSettleSelectorScanStats) {
      window.__ctripSettleSelectorScanStats = [];
    }
    return window.__ctripSettleSelectorScanStats;
  };
  const querySelectorAllWithStats = (selector, label) => {
    const startedAt = getPerformanceNow();
    const elements = Array.from(document.querySelectorAll(selector));
    const elapsedMs = Math.max(0, getPerformanceNow() - startedAt);
    getSelectorScanState().push({
      label,
      selector,
      elapsedMs,
  ...
  const drainSelectorScanStats = () => {
    const scans = getSelectorScanState();
    window.__ctripSettleSelectorScanStats = [];
    return scans;
  };
  const summarizeSelectorScanStats = (scans) => {
    let selectorScanElapsedMs = 0;
    let slowest = null;
    for (const scan of scans) {
      const elapsedMs = Number(scan.elapsedMs || 0);
      selectorScanElapsedMs += elapsedMs;
      if (!slowest || elapsedMs > Number(slowest.elapsedMs || 0)) {
        slowest = scan;
      }
    }
    return {
      selectorScanCount: scans.length,
      selectorScanElapsedMs: Math.round(selectorScanElapsedMs),
      slowestSelectorScanLabel: slowest ? String(slowest.label || '') : '',
      slowestSelectorScanElapsedMs: slowest ? Math.round(Number(slowest.elapsedMs || 0)) : 0,
      slowestSelectorScanCandidateCount: slowest ? Number(slowest.candidateCount || 0) : 0
    };
  };
  const getRoomSectionBounds = () => {
    ...
    const elements = querySelectorAllWithStats(
      [
        'section',
        'article',
        'div',
  ...
  const getExpandScanElements = (includeGenericText = true, options = {}) => {
    ...
    const elements = querySelectorAllWithStats(
      (includeGenericText ? FALLBACK_EXPAND_SCAN_SELECTORS : EXPLICIT_EXPAND_SCAN_SELECTORS)
        .join(', '),
      includeGenericText ? 'expand_fallback_scan' : 'expand_explicit_scan'
    );
  ...
  const finishStats = (before, extra = {}) => {
    ...
    const selectorScanStats = summarizeSelectorScanStats(drainSelectorScanStats());
    return {
      ...
      selectorScanCount: extra.selectorScanCount || selectorScanStats.selectorScanCount || 0,
      selectorScanElapsedMs:
        extra.selectorScanElapsedMs || selectorScanStats.selectorScanElapsedMs || 0,
      slowestSelectorScanLabel:
        extra.slowestSelectorScanLabel || selectorScanStats.slowestSelectorScanLabel || '',
      slowestSelectorScanElapsedMs:
        extra.slowestSelectorScanElapsedMs || selectorScanStats.slowestSelectorScanElapsedMs || 0,
      slowestSelectorScanCandidateCount:
        extra.slowestSelectorScanCandidateCount ||
        selectorScanStats.slowestSelectorScanCandidateCount ||
        0,
  ...
  const closeReviewPanels = () => {
    ...
    for (const sel of closeSelectors) {
      try {
      const els = querySelectorAllWithStats(sel, 'close_panel_scan');
        for (const el of els) {
  ...
  const scanStartedAt = getPerformanceNow();
  ...
  result.clickScanElapsedMs = getPerformanceNow() - scanStartedAt;
    return result;
  };
  ...
  const getScrollableContainers = () => {
    const roomSectionBounds = getRoomSectionBounds();
  return querySelectorAllWithStats('*', 'scroll_container_scan').filter(el => {
  ...
  return {
    ...
    getDocumentHeight,
    readBodyText,
    getRoomSectionBounds,
    isElementInRoomSection,
    querySelectorAllWithStats,
    drainSelectorScanStats,
    summarizeSelectorScanStats,
    collectStats,
    getExpandScanElements,
    scanExpandElements,
    clickExpandButtons,
    closeReviewPanels,
    scrollAllContainers,
    selectLikelyRoomScrollContainers,
    finishStats,
    isCollapseOnlyRoomToggle
  };
}

const SESSION_SETTLE_HELPER_NAMES = [
  "sleep",
  "waitForDomIdle",
  "getDocumentHeight",
  "readBodyText",
...
function buildSessionSettleHelpersSource() {
  const helperBindings = SESSION_SETTLE_HELPER_NAMES.join(',\n  ');
  return `
const {
  ${helperBindings}
} = (${createSessionSettleBrowserHelpers.toString()})();
`;
}

function buildSessionSettleStepExpression(body) {
  return `(async () => {
  ${buildSessionSettleHelpersSource()}
  drainSelectorScanStats();
  ${body}
})()`;
}
