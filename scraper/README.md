...
- **多房型输出**：`selectMatchingRooms()` 筛选所有符合条件房间（有价格、standard_title 非空、score >= 0、走廊/过道窗视为无窗；人数默认精确匹配模板，另允许 `room_count = 1` 时保留 `occupancy = 2` 的大床房，若开启“3人模板时额外保留4人房”则允许 `3 -> 4`）
...
模板与 CLI 的采集人数当前仍只支持 `1-3`；如果 `1` 人模板保留了网页显示可住 `2` 人的大床房，记录里的 `room_count` 写 `2`；如果需要额外保留 `4` 人房，请使用 `3` 人模板并开启“3人模板时额外保留4人房”。
