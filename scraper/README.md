...
- 列表页先做前筛，再逐个进入详情页复用原采集链路；应用内前筛支持每日人均价格、星级、排序、免费取消、点评数、评分档位、住宿类型、房型、客房特色、特色主题，每日人均价格会按模板入住天数和 room_count 换算为携程 URL 总价筛选；应用内列表过滤只保留目标采集数量，候选不足时继续采集已确认候选并在结果里给出数量提示
...
```bash
node src/cli.js --url "携程列表页链接" --templateName 模板名 --priceMin 50 --priceMax 200 --starLevels "3,4" --sortMode review_high --freeCancel true --reviewCountMin 500 --ctripScoreMin 4.5 --accommodationTypeMode exclude --accommodationTypes "民宿,公寓" --roomTypes "双床房" --roomFeatures "家庭房" --featureThemes "电竞酒店" --auto-edge --edge-user-data-dir ./state/edge-profile --edge-profile-directory Default --edge-debugging-port 9222
```
...
2. **人数按模板匹配，1 人模板有大床例外** — 模板和 CLI 的采集人数支持 `1-4`。当 `room_count = 1` 时，保留 `occupancy = 1` 的房型，也允许 `occupancy = 2` 且标准化后为“大床房”的房型；`occupancy = 2` 的双床房、家庭房、三床房等非大床房仍排除。其他模板默认只保留 `occupancy == 模板 room_count` 的房型；`room_count = 4` 会直接匹配可住 4 人房型。如果比较助手设置里开启“3人模板时额外保留4人房”，则在 `room_count = 3` 时也允许保留 `occupancy = 4` 的房型；写回比较助手的酒店记录 `room_count` 应按房型实际可住人数填写
...
- **多房型输出**：`selectMatchingRooms()` 筛选所有符合条件房间（有价格、standard_title 非空、score >= 0、走廊/过道窗视为无窗；人数默认精确匹配 `1-4` 人模板，另允许 `room_count = 1` 时保留 `occupancy = 2` 的大床房，若开启“3人模板时额外保留4人房”则允许 `3 -> 4`）
...
- 源码默认包含一个高德 Key；使用 `npm run build:no-key` 打包时，安装包内的采集器默认 Key 会被移除。
...
模板与 CLI 的采集人数支持 `1-4`；如果 `1` 人模板保留了网页显示可住 `2` 人的大床房，记录里的 `room_count` 写 `2`；`4` 人模板会直接匹配网页显示可住 `4` 人的房型。如果使用 `3` 人模板但也想额外保留 `4` 人房，可以开启“3人模板时额外保留4人房”。
...
src/
  ...
  ctrip-list.js       # 多 URL 与列表页展开、前筛、Edge-CDP 列表兜底
