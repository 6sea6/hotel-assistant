...
test('findTemplateInStore only matches explicit template id or exact normalized name', withBridge(async ({ bridge }) => {
  const store = {
    templates: [
      {
        id: 1,
...
test('appendHotelsToStore replaceExistingGroup writes grouped hotels without flattening schema', withBridge(async ({ bridge }) => {
  const templateInfo = {
    id: 2001,
    name: 'bw',
    destination: '上海国家会展中心',
...
test('appendHotelsToStore replaceExistingGroup preserves untouched rooms and only updates duplicates from current write', withBridge(async ({ bridge }) => {
  const templateInfo = {
    id: 2001,
    name: 'bw',
    destination: '上海国家会展中心',
...
test('appendHotelsToStore overwriteExistingGroup replaces all rooms in the matched hotel group', withBridge(async ({ bridge }) => {
  const templateInfo = {
    id: 2001,
    name: 'bw',
    destination: '上海国家会展中心',
...
test('pointer file overrides default compare app data folder', withBridge(async ({ appDataRoot, tempRoot, bridge }) => {
  ...
  assert.equal(
    bridge.getCompareAppStorePath(),
    path.join(customDataFolder, 'hotel-data.json')
  );
}));

test('explicit compare app data dir override wins over pointer', withBridge(async ({ appDataRoot, tempRoot }) => {
  const customDataFolder = path.join(tempRoot, 'custom-data-folder');
  const explicitDataFolder = path.join(tempRoot, 'explicit-data-folder');
  fs.mkdirSync(customDataFolder, { recursive: true });
  fs.mkdirSync(explicitDataFolder, { recursive: true });
  fs.writeFileSync(path.join(explicitDataFolder, 'hotel-data.json'), JSON.stringify({ hotels: [], templates: [] }), 'utf-8');
  fs.writeFileSync(
    path.join(appDataRoot, 'hotel-app-pointer.json'),
    JSON.stringify({ dataFolder: customDataFolder }, null, 2),
    'utf-8'
...
test('workspace compare app folder wins over pointer in local development mode', withBridge(async ({ appDataRoot, tempRoot }) => {
  ...
  fs.writeFileSync(path.join(workspaceFolder, 'hotel-data.json'), JSON.stringify({ hotels: [], templates: [] }), 'utf-8');
  fs.writeFileSync(
    path.join(appDataRoot, 'hotel-app-pointer.json'),
...
test('loadCompareAppStore normalizes settings and removes deprecated autoMatchTemplate flag', withBridge(async ({ bridge }) => {
...
test('appendHotelsToStore drops legacy unknown fields during duplicate merges', withBridge(async ({ bridge }) => {
  const storePath = bridge.getCompareAppStorePath();
  fs.mkdirSync(path.dirname(storePath), { recursive: true });
  fs.writeFileSync(storePath, JSON.stringify({
    hotels: [
