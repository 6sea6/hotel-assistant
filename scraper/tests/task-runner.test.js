...
test('runHotelImportTask reuses prepared context for batch detail items', async () => {
  ...
  mockedPaths.push(installMock('../src/compare-app-bridge', {
  ...
  mockedPaths.push(installMock('../src/ctrip-list', {
  ...
  mockedPaths.push(installMock('../src/ctrip-scraper', {
  ...
  mockedPaths.push(installMock('../src/amap', {
  ...
  mockedPaths.push(installMock('../src/hotel-record', {
  ...
  mockedPaths.push(installMock('../src/review-input', {
    buildReviewInput: () => ({
      selectionLogs: [],
      rejectedRoomTypes: [],
      normalizeLogs: []
  ...
  mockedPaths.push(installMock('../src/template-loader', {
...
test('runHotelImportTask completes a single detail URL without batch writeResult scope leak', async () => {
  ...
  try {
    ...
    assert.ok(events.some((event) => event.type === 'task:done' && event.details && event.details.wrote === false));
  } finally {
    clearModules([taskRunnerPath, ...mockedPaths]);
    fs.rmSync(tempDir, { recursive: true, force: true });
  }
