  ...
  const el = {
    ...
    querySelector(sel) {
      ...
      for (const child of children) {
        if (
          sel.startsWith('[data-custom-select-option') &&
          child.dataset?.customSelectOption === 'true'
        ) {
          return child;
        }
        if (
          sel.includes('is-active') &&
          child.dataset?.customSelectOption === 'true' &&
          child.classList?._classes?.has('is-active')
        ) {
          return child;
        }
        if (
          ...
          child.className &&
          sel.startsWith('.') &&
          child.classList.contains(sel.slice(1))
        ) {
          return child;
        }
      }
...
test('custom-select: selected option has is-selected class', async () => {
  ...
  const options = menu.querySelectorAll('[data-custom-select-option="true"]');
  assert.equal(
    options[0].classList.contains('is-selected'),
    false,
    'first option should not be selected'
  );
  assert.equal(
    options[1].classList.contains('is-selected'),
    true,
...
test('custom-select: destroyCustomSelect cleans up events and removes instance', async () => {
  installMockDom();
  const { enhanceCustomSelect, destroyCustomSelect, getCustomSelectInstance } =
    await loadCustomSelectModule();
  ...
  assert.equal(
    ...
    false,
    'native class should be removed'
  );
...
test('custom-select: refreshCustomSelect on data-custom-select="false" select still works', async () => {
  installMockDom();
  const { enhanceCustomSelect, refreshCustomSelect, getCustomSelectInstance } =
    await loadCustomSelectModule();
...
test('custom-select: aiTemplateSelect not found by findTargetSelects in auto mode', async () => {
  ...
  const aiSelect = createMockSelect('aiTemplateSelect', [{ value: 't1', text: 'T1' }], {
    customSelect: 'false',
    customSelectAuto: 'false'
  });

  // Simulate a normal select with class "input"
  const normalSelect = createMockSelect('normalSelect', [{ value: 'n1', text: 'N1' }]);
  ...
  assert.equal(
    getCustomSelectInstance(aiSelect),
    null,
    'aiTemplateSelect should not be auto-enhanced'
  );
...
test('custom-select: data-custom-select-option used in keyboard navigation', async () => {
  ...
  const options = menu.querySelectorAll('[data-custom-select-option="true"]');
  assert.equal(
    options[0].classList.contains('is-active'),
    true,
    'first option should have is-active after first ArrowDown'
  );
});
...
test('custom-select: setupCustomSelects can recover stale ready state', async () => {
  ...
  const select = createMockSelect('staleSelect', [{ value: 'a', text: 'A' }]);
...
test('custom-select: refreshCustomSelects auto enhances newly inserted select.input', async () => {
  ...
  assert.ok(
    getCustomSelectInstance(select),
    'new select.input should be enhanced by refreshCustomSelects auto'
  );
...
test('custom-select: refreshCustomSelect with auto option enhances single select.input', async () => {
  ...
  const select = createMockSelect('singleNew', [{ value: 'p', text: 'P' }]);
  select.classList.add('input');
  ...
  assert.ok(
    getCustomSelectInstance(select),
    'select.input should be enhanced by refreshCustomSelect with auto'
  );
});
...
test('custom-select: refreshCustomSelects auto still excludes aiTemplateSelect', async () => {
  ...
  const normalSelect = createMockSelect('normalSel', [{ value: 'n', text: 'N' }], {
    customSelect: 'true'
  });
  ...
  assert.equal(
    getCustomSelectInstance(aiSelect),
    null,
    'aiTemplateSelect should still be excluded'
  );
...
test('custom-select: openMenu uses getAnimationFrame fallback when requestAnimationFrame missing', async () => {
  ...
  const select = createMockSelect('rafTest', [{ value: 'a', text: 'A' }]);
