...
这是一个基于 Electron 的本地宾馆比较助手，用于录入、筛选、排序和导出宾馆数据，并支持模板复用、模板时间范围展示、个性化主题与应用图标切换、采集助手和中文化调试。
...
5. 采集助手内置携程酒店采集链路，并保留 AI 兜底分析能力
...
12. 完整版安装包会内置项目 `scraper` 目录中的采集器资源，基础版只包含比较助手本体
13. 完整版首次启动会准备采集器运行目录和采集规则说明文件，避免依赖构建机用户目录
