...
function createAiService({ dataService, windowService, hotelTaskRunner = null }) {
  ...
  async function analyzeCollection(payload = {}) {
    const taskId = String(payload.taskId || '').trim();
    if (!taskId) {
      throw new Error('缺少 taskId，无法进行 AI 分析。');
    }
    ...
    const reviewInput = getReviewInputForTask(taskId, payload.userConcern);
    const reviewMessages = [
      ...
      {
        role: 'user',
        content: buildReviewUserPrompt(reviewInput)
      }
    ];
    const assistantMessage = await requestChatCompletion(config, reviewMessages, [], {
      maxTokens: 4096
    });
    ...
    const analysis = validateAnalysisResult(rawAnalysis, reviewInput);
    const reviewId = randomUUID();

    state.pendingReviews.set(reviewId, {
      id: reviewId,
      taskId,
      outputFingerprint: reviewInput.taskMeta.outputFingerprint,
      reviewInput,
      analysis,
  ...
  async function applyCollectionReview(payload = {}) {
    const reviewId = String(payload.reviewId || '').trim();
    const review = state.pendingReviews.get(reviewId);
    if (!review) {
      throw new Error('未找到 AI 分析结果，请重新分析后再写入。');
    }
    if (!review.analysis || !review.analysis.canApply) {
    ...
    state.pendingReviews.delete(reviewId);

    emitTaskEvent({
      type: 'review:apply-done',
      message: 'AI 复核结果已覆盖写入',
      taskId: review.taskId,
      details: {
